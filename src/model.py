...
class GPTLightningModule(pl.LightningModule):
    def __init__(self, vocab_size, cfg):
        ...
        self.blocks = nn.Sequential(*[
            Block(cfg.model.n_embd, cfg.model.n_head, cfg.model.block_size, cfg.model.dropout) 
            for _ in range(cfg.model.n_layer)
        ])
        self.ln_f = nn.LayerNorm(cfg.model.n_embd)
        self.lm_head = nn.Linear(cfg.model.n_embd, vocab_size)
    ...
    def configure_optimizers(self):
        return torch.optim.AdamW(self.parameters(), lr=self.cfg.model.learning_rate)
